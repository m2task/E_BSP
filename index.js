document.addEventListener('DOMContentLoaded', () => {
    ...
    // const cropButton = document.getElementById('crop-button'); // 不要になったため削除
    ...
    // --- Card Cropping Function ---
    function executeCardCropping() {
        console.log('3. executeCardCropping function called.'); // 追加
        if (!imageState.isLoaded) {
            console.log('3a. executeCardCropping: imageState.isLoaded is false.'); // 追加
            alert('まず画像を選択してください。');
            return;
        }
        if (typeof cv === 'undefined' || !cv.imread) {
            console.log('3b. executeCardCropping: OpenCV not loaded or imread not available.'); // 追加
    ...
    imageLoader.addEventListener('change', (e) => {
        console.log('1. imageLoader change event fired.'); // 追加
        deck = [];
        currentEditingDeckName = null;
        renderEditedDeck();
        ...
        if (file) {
            // cropButton.classList.remove('hidden'); // 不要になったため削除

            const reader = new FileReader();
            reader.onload = (event) => {
                sourceImage.src = event.target.result;
                sourceImage.onload = () => {
                    console.log('2. sourceImage onload event fired.'); // 追加
                    imageState.isLoaded = true;

                    const initialScale = 0.2; // 初期表示倍率を直接指定
